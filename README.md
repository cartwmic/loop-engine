...
Current release is v0.8.0 (`MIT OR Apache-2.0`). The living product requirements are [docs/PRD.md](docs/PRD.md). Agent CLI semantics are [docs/agent-usage.md](docs/agent-usage.md). Checkout operating rules for agents are [AGENTS.md](AGENTS.md).
...
```sh
mise use --global 'github:cartwmic/loop-engine[exe=loop-engine]@v0.8.0'
for app in software-change-provider policy-document-provider research-provider; do
  curl --proto '=https' --tlsv1.2 -LsSf \
    "https://github.com/cartwmic/loop-engine/releases/download/v0.8.0/$app-installer.sh" | sh
done
```
