...
Use `fan-out` **ad hoc** when you want parallel worker CLIs without a run: pass `--instructions FILE` and do not send an invoke packet on stdin. Without `preamble`, worker stdin is byte-identical to the instruction-file bytes. With `preamble`, stdin is the preamble bytes unchanged, one LF appended only if the preamble does not already end in LF, literal `---\n\n`, then the unchanged instruction-file bytes. Ad hoc mode has no invoke packet and inserts no artifact context.

When a work slot is frozen to `loop-engine` args that begin with `fan-out`, the legal start remains `loop-engine invoke RUN_ID SLOT_ID`. Do not call `fan-out` yourself for that slot. Bound mode rejects `--instructions` and reads the unchanged five-key packet. A worker without `preamble`, including an `output_schema`-only worker, receives byte-identical legacy stdin. With `preamble`, stdin is exactly: preamble bytes unchanged; one LF only if absent; compact one-key JSON `{"artifact_root": packet.artifact_root}`; one LF; literal `---\n\n`; then the unchanged legacy state instruction body with its existing LF normalization, blank line, and `run_id`/`slot_id`/resolved `artifact_root` trailer. Only bound preamble mode adds that context. It contains no `capture_dir` or duplicate run/slot identity, and adds no new invoke-packet keys or digest input.

Bound mode writes per-worker stdout/stderr under `packet.capture_dir/0/`, `1/`, … and writes `summary.json` in worker order. For `output_schema`, stdout must be a bare JSON object or the sole fenced `json` object amid prose; fan-out checks only required top-level key presence. A contracted summary entry preserves `command`, `args`, true `exit_code`, `stdout_path`, and `stderr_path`, adds `status` (`succeeded` or `failed`), and adds `conformance_error` only on failure. Workers without `output_schema` preserve the legacy summary shape by omitting both new fields. Any nonconformance fails the collector even when the worker exits 0, but `summary.json` is written before failure so the driver can inspect it and captured stdout before stderr. Ordinary inner nonzero exits retain legacy collector behavior. Exit 0 and key presence do not establish semantic validity; the driver still validates and triages the values.
