...
A deterministic consumer should notify the driver on a selected source's `completion` or `attention`, preserving `source`, `boundary.reason` and evidence locators. The monitor keeps following until interrupted; stop the observer when no longer needed, not the observed work. Inspect the explicit `workflow_lane`, `execution`, `worker`/`worker_lane`, `conformance`, `acceptance`, `evidence`, `freshness`, and `uncertainty` lanes separately. `acceptance: unknown` is the safe result when provider/driver evidence is not present; only an explicit attributed accepted/rejected decision in a durable context record or dated `--observation` file populates it. Completion, conformance, and an observation without that decision do not promote it. Use `next_action` only as guidance: it can say wait, inspect, or help but cannot authorize a mutation. After a meaningful observed change, read the status/monitor packet and post a concise update in the active Pi conversation before the first later wait, inspect, or help decision. Name both the observed change and the needed action or owner decision; a machine attention/completion line is not that owner update. Suppress the update when the source is unchanged. Neither completion nor an elapsed attention deadline approves, retries or cancels anything. Re-read actionable show before acting. Use `invocation-progress RUN [INVOCATION]` for specific graph/trace diagnosis: helper `reaped` is not inner exit 0 or provider acceptance; its `visibility` lanes remain execution/detail evidence, not workflow or acceptance authority.
