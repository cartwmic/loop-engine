...
FINAL_MATRIX_COMMANDS = [
    ...
    r"bash -o pipefail -c 'env -u BOOKENDS_BYPASS python3 scripts/software-change-journey.py --mode source --engine target/debug/loop-engine --provider target/debug/software-change --data-root $PWD --work-root ${TMPDIR:-/tmp}/loop-engine-software-change-journey --profile crates/software-change-provider/data/configs/high-rigor.json --traversal-depth full | tee /tmp/loop-engine-software-change-source.log && rg -q contracted\ fan-out\ failure /tmp/loop-engine-software-change-source.log && rg -q Package\ 7b\ review-candidates\ scenario\ passed: /tmp/loop-engine-software-change-source.log && rg -q bookends-enabled\ external\ scenario\ passed: /tmp/loop-engine-software-change-source.log && rg -q LE-2\ topology\ scenarios\ passed:\ malformed\ rejected,\ cyclic\ topology\ accepted /tmp/loop-engine-software-change-source.log && rg -q LE-11\ frozen-run\ scenario\ passed:\ changed\ describe\ did\ not\ alter\ show /tmp/loop-engine-software-change-source.log && rg -q LE-12\ unsupported-action\ scenario\ passed:\ explicit\ and\ operational\ errors\ preserved\ state /tmp/loop-engine-software-change-source.log && rg -q LE-13\ final-state\ scenario\ passed:\ outgoing\ transition\ rejected\ before\ run\ creation /tmp/loop-engine-software-change-source.log && rg -q LE-14\ initially-final\ scenario\ passed:\ run\ created\ final /tmp/loop-engine-software-change-source.log && rg -q LE-15\ terminal-mutation\ scenario\ passed:\ append/event/terminate\ rejected\ without\ history\ change /tmp/loop-engine-software-change-source.log'",
