use research_provider::embedded_data::FILES;
...
fn collect_file(root: &Path, path: &Path, files: &mut BTreeMap<String, Vec<u8>>) {
    let relative = path
        .strip_prefix(root)
        .expect("file must be beneath collection root")
        .to_string_lossy()
...
fn on_disk_data_files() -> BTreeMap<String, Vec<u8>> {
    let root = repo_root();
    let crate_root = root.join("crates/research-provider");
    let mut files = BTreeMap::new();
    collect_file(&root, &crate_root.join("AGENTS.md"), &mut files);
    collect_file(&root, &crate_root.join("README.md"), &mut files);
    collect_files(&root, &crate_root.join("data"), &mut files);
    collect_files(&root, &crate_root.join("skills"), &mut files);
    files
}
...
fn temporary_path(label: &str) -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "research-provider-embedded-{label}-{}-{index}",
        std::process::id()
    ))
}

fn dump(destination: &Path) -> std::process::Output {
    Command::new(workspace_integration::binary("research"))
        .arg("data-dump")
        .arg(destination)
        .output()
...
fn data_dump_matches_tree_and_refuses_existing_targets_without_writing() {
    ...
    let existing_target = destination.join("crates/research-provider/data/configs/standard.json");
