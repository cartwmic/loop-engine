# Verification artifact (generate-PRD)

Write `verification.json` as adversarial claim checking against gathered repository sources. The provider does not judge claim truth.

Claims concern proposed living-PRD requirements and the reviewable repository evidence that supports each one.
