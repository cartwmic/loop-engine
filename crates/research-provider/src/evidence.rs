...
mod tests {
    ...
    const SUBJECT: &str = "verification.json";
    const GATE: &str = "verify";
    ...
    fn config_with_two_gates() -> crate::config::ValidatedConfig {
        let mut config = json!({
            ...
            "review_policies": {
                ...
                "synthesize": [{"id": "other-axis", "description": "other"}]
            },
            "artifact_schemas": {
                SUBJECT: metadata_schema(),
                "report.json": metadata_schema()
            }
        });
        config["review_policies"][GATE][0]
            .as_object_mut()
    ...
    fn evaluate(
        ...
        evaluate_evidence(
            ...
            config.axes_for_gate(GATE).expect("verify axes"),
    ...
    fn structural_conformance_negative_matrix_covers_all_rules() {
        ...
        let cases = vec![
            ...
            ("mismatched subject", "subject", json!("brief.json"), true),
