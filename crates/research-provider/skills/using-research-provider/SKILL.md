...
1. `show` — read `current_state`, `current_state_instructions`, frozen `initial_input` (including `work_slot_bindings`, `review_policies`, `artifact_schemas`), `work_slots`, and `work_slot_invocations` (`assignment_selection`, per-worker assignment/selected-attempt identity, `overlay_meaning`, `elapsed_ms`, `remaining_allowed_ms`, `capture_dir`, `inner_workers`), plus the provider-free `change_report`. This observation arms only the current state visit. Repeat this step after every transition before the next append, event, invoke, or terminate.
