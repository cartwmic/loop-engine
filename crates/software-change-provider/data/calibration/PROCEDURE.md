...
Every subject fixture and every `intent_revision`, `design_revision`, and `plan_revision` link uses neutral revision `r15`. For each pair, defective subjects receive the same pass predecessor bytes so review isolates subject material. The current shipped versions are `minimal-11`, `standard-11`, and `high-rigor-11` (contract v3). Historic `minimal-10`, `standard-10`, and `high-rigor-10` rows remain in the manifest and are never rewritten as new rows. Stage-aware rows are keyed by profile, gate, axis, review stage, and fixture identity; the ordinary corpus keeps its paired PASS/FAIL rows, while the new intent calibration corpus may retain several named supplied contrasts for one axis so coherent, bundled, fragmented, impractical, owner-clear, and unstated-background cases remain separately inspectable. Keep each supplied PASS/FAIL expectation unchanged while re-assessing changed inputs. The AC-N criterion spine is part of the shipped artifact schema; overlay-off calibration uses no `prd_traceability` disposition, while overlay-on journey proof supplies the optional Bookends `linked-live`, `candidate`, and `not-applicable` annotations separately; `not-applicable` does not waive or fulfill its criterion. After any supplied prompt, protocol, template, schema, or fixture change, rehash affected current-version rows and reset them to pending until a fresh owner review is completed; do not rewrite frozen historic rows, change row keys, or change expected values to force agreement.
...
Any supplied-byte change invalidates every current row whose source stream contains that byte; unrelated rows remain scoped to their own source set. A needed policy, schema, link, or prompt change bumps that profile `config_version`, requires rehashing the affected current manifest keys, and requires fresh semantic owner review. `input_sha256` is mechanical identity, not semantic review proof. No provider runtime path reads or interprets it.

Rows with the historic `minimal-10`, `standard-10`, or `high-rigor-10` config version are frozen evidence snapshots. Preserve their exact row order, source hash, returned result, model, invocation, and attestation identity; do not recompute those hashes from a successor profile or use their attestations to satisfy a v11 row. Current source-record hashing and packet preparation apply only to `minimal-11`, `standard-11`, and `high-rigor-11`; the preparation utility refuses legacy row keys against those current profiles.
...
Never change `expected`, row key, config, fixture, or coverage to force agreement. Keep returned evidence outside manifest when needed; `example-evidence.json` is illustrative R25 data, not attestation. The historic v10 rows retain 184 PASS/184 FAIL expectations, the v11 copies of the existing corpus retain another 184 PASS/184 FAIL expectations, and the new v11 intent corpus adds 12 PASS/16 FAIL expectations; fixture material remains unchanged while rows are pending.
