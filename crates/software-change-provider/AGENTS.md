...
Shipped profile versions currently in-tree: `minimal-8`, `standard-8`, `high-rigor-8`. Evidence `config_version` must match the run's frozen value, not whatever file is currently shipped. Intent acceptance uses the closed run-local `{id, statement}` AC-N criterion spine; downstream AC-N references are optional and are not a complete matrix. With `extra.bookends.enabled: true`, each current criterion carries exactly one `prd_traceability` disposition: linked-live, candidate, or not-applicable; not-applicable never waives or fulfills the criterion, and a current candidate blocks Bookends-enabled final completion. Shipped profiles omit `work_slot_bindings`; bound workers are opt-in, and review bindings use the skill's deterministic per-axis constructor. Gate keys are `intent-review` and `validation-review` (not `intent` or `validation`). Standard and high-rigor ship 1:1 challenge counterparts (`required_authors` 1) on every parent review list under unchanged `*-adversarial-review` IDs; minimal ships no challenge lists.
