...
`software-change --help`/`-h` names `describe`, `evaluate`, `data-dump`, `checkpoint`, and `run-plan-graph`. `--version`/`-V` prints the Cargo package version. Hidden `stdin-exec` uses the same argv as `loop-engine stdin-exec` and is omitted from `--help` and `--version`. Plan-graph uses `--exit-mode propagate` only. `run-plan-graph` fail-closes if PATH `dagu` is missing, not runnable, or older than 2.14.0, naming the path or PATH miss and required version, before any worker spawn. Isolated home is `capture_dir/dagu-home/` with locator `capture_dir/dagu-locator.json` keys `dagu_home`, `dag_name`, and `run_name` (`plan-graph-<capture-dir-name>`). The facade emits a Dagu `type:graph` (`max_active_steps` 4, fail-fast, no `continue_on`); per-step progress is `dagu status` / `dagu history` against that home. A mandatory `summarizer` step is the sole writer of `artifact_root/implementation-report.json`; after successful implementation it also produces the provider-generated implementation checkpoint. Provider packages do not ship `dagu`. Review slots carry the current finding-ledger context plus their assigned axis; the implementation slot forwards ledger context to enrich exact plan tasks under `finding_context` while preserving inner `{artifact_root, task}` stdin and full-DAG execution. Advisory-finding-proposal records are inert until the driver accepts or edits them into a finding-ledger snapshot. `data-dump DIR` materializes embedded data and refuses to overwrite existing target files.
