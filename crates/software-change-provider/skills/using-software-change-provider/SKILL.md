...
**Concurrency:** `loop-engine fan-out --max-active N` omitted stays uncapped; set N is at most N worker steps. `software-change run-plan-graph --max-active N` omitted stays 4 ordinary plan tasks; set N is at most N ordinary plan tasks; the summarizer still runs after those tasks.
