...
mod tests {
    ...
    fn valid_config_parses_into_semantically_keyed_surface() {
        ...
        config["revision_links"] = json!([
            {"from": "intent.json", "field": "base_revision", "to": "design.json"}
        ]);
        config["artifact_schemas"]["design.json"] = revision_schema();
    ...
    fn missing_review_policies_names_shipped_configs() {
        ...
        assert!(message.contains("high-rigor"));
    }
    ...
    fn link_to_schema_must_supply_revision_clause() {
        ...
        config["artifact_schemas"] = json!({
            "design.json": revision_schema(),
            "intent.json": {"type": "object"}
        });
        config["revision_links"] = json!([
            {"from": "design.json", "field": "intent_revision", "to": "intent.json"}
        ]);
    ...
    fn bad_schema_keyword_is_rejected_by_generic_validator() {
        let mut config = empty_config();
        config["artifact_schemas"] = json!({
            "intent.json": {"type": "object", "not_a_keyword": true}
        });
    ...
    fn malformed_revision_link_missing_field_is_rejected() {
        ...
        config["artifact_schemas"] = json!({
            "design.json": revision_schema(),
            "intent.json": revision_schema()
        });
        config["revision_links"] = json!([
            {"from": "design.json", "to": "intent.json"}
        ]);
    ...
    fn malformed_revision_link_wrong_type_is_rejected() {
        ...
        config["revision_links"] = json!([
            {"from": "design.json", "field": 3, "to": "intent.json"}
        ]);
    ...
    fn malformed_revision_link_extra_field_is_rejected() {
        ...
        config["revision_links"] = json!([
            {"from": "design.json", "field": "intent_revision", "to": "intent.json", "extra": true}
        ]);
    ...
    fn malformed_revision_link_unknown_subject_is_rejected() {
        let mut config = empty_config();
        config["revision_links"] = json!([
            {"from": "unknown.json", "field": "intent_revision", "to": "intent.json"}
        ]);
