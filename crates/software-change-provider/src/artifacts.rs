...
fn is_report_subject(subject: &str) -> bool {
    matches!(
        subject,
...
mod tests {
    ...
    impl TestDir {
        fn new() -> Self {
            ...
            let path = std::env::temp_dir().join(format!(
                "loop-engine-software-change-artifacts-{}-{suffix}",
                std::process::id()
            ));
    ...
    fn reads_valid_json_under_root() {
        ...
        root.write_json("intent.json", &json!({"revision": "1"}));

        let outcome = read_artifact(Some(&root.path_value()), "intent.json");
        let ArtifactReadOutcome::Present(document) = outcome else {
            panic!("expected present artifact");
        };
        assert_eq!(document.subject(), "intent.json");
    ...
    fn missing_artifact_is_deny_not_found() {
        let root = TestDir::new();
        let deny = assert_deny(read_artifact(Some(&root.path_value()), "intent.json"));
        assert!(matches!(deny, ArtifactReadDeny::NotFound { .. }));
        assert_eq!(deny.subject(), "intent.json");
    }

    #[test]
    fn unparseable_json_is_deny() {
        let root = TestDir::new();
        fs::write(root.path.join("intent.json"), b"not json").unwrap();
        let deny = assert_deny(read_artifact(Some(&root.path_value()), "intent.json"));
    ...
    fn missing_relative_and_non_string_roots_are_evaluation_errors() {
        ...
        assert_eval_error(read_artifact(Some(&missing), "intent.json"));
        assert_eval_error(read_artifact(Some(&json!("relative-root")), "intent.json"));
        assert_eval_error(read_artifact(Some(&json!(42)), "intent.json"));
        assert_eval_error(read_artifact(None, "intent.json"));
    }
    ...
    fn file_root_and_directory_target_are_evaluation_errors() {
        ...
        assert_eval_error(read_artifact(Some(&file_root_value), "intent.json"));

        fs::create_dir(root.path.join("intent.json")).unwrap();
        assert_eval_error(read_artifact(Some(&root.path_value()), "intent.json"));
    }
    ...
    fn invalid_subject_path_is_evaluation_error() {
        let root = TestDir::new();
        assert_eval_error(read_artifact(Some(&root.path_value()), "../intent.json"));
        assert_eval_error(read_artifact(
    ...
    fn revision_link_passes_when_source_field_matches_target_revision() {
        let root = TestDir::new();
        root.write_json("intent.json", &json!({"revision": "r1"}));
        let config = link_config();
        let link = &config.links_from("design.json")[0];

        let result = check_revision_link(
            Some(&root.path_value()),
            "design.json",
            &json!({"intent_revision": "r1"}),
            link,
        );
    ...
    fn revision_link_reports_mismatch() {
        let root = TestDir::new();
        root.write_json("intent.json", &json!({"revision": "r2"}));
        let config = link_config();
        let link = &config.links_from("design.json")[0];
        ...
        assert_eq!(from_subject, "design.json");
        assert_eq!(from_path, "/intent_revision");
        assert_eq!(to_subject, "intent.json");
    ...
    fn revision_link_reports_missing_or_non_string_source_field() {
        ...
        let missing =
            check_revision_link(Some(&root.path_value()), "design.json", &json!({}), link);
        assert!(matches!(
            missing,
    ...
    fn revision_link_reports_missing_or_non_string_target_revision() {
        ...
        root.write_json("intent.json", &json!({"other": "r1"}));
        ...
        assert!(matches!(
            ...
            LinkCheckOutcome::Violation(LinkViolation::TargetRevisionMissing {
                ...
                ref to_subject,
                ref to_path,
            }) if from_subject == "design.json"
                && from_path == "/intent_revision"
                && to_subject == "intent.json"
                && to_path == "/revision"
        ));
    ...
    #[test]
    fn extraction_round_trip_includes_report_coverage() {
        let metadata = extract_metadata(
            ...
            &json!({
                ...
                "coverage": {
                    "commit": "abc123",
                    "documents": [{"path": "docs/PRD.md", "revision": "doc-2"}]
                }
            }),
        )
        ...
        let coverage = metadata.coverage().expect("coverage");
    ...
    #[test]
    fn report_without_coverage_is_metadata_invariant_failure() {
        ...
        .expect_err("report coverage is required");
    ...
    fn artifact_read_does_not_write() {
        let root = TestDir::new();
        root.write_json("intent.json", &json!({"revision": "1"}));
        let before = fs::read(root.path.join("intent.json")).unwrap();
        let _ = read_artifact(Some(&root.path_value()), "intent.json");
        let after = fs::read(root.path.join("intent.json")).unwrap();
