...
- `evaluate` checks the exact transition, validates configured artifact schemas and revision links, then evaluates externally supplied review evidence and driver-authored finding-ledger snapshots.

The frozen requirement record this crate's acceptance suite traces to (R1–R28, A1–A16, including amendments) lives at [`docs/prd.md`](docs/prd.md).
...
Evidence denial details separate current blockers (`details.diagnostics`) from stale or stale-config recovery context (`details.informational`). A claim about a bound worker includes `originating_output` with the exact `invocation_id`, `assignment_id`, `selected_attempt`, `sha256`, and `path` from public `show`; append rejects identities that do not match the durable engine-selected record. Provider evaluation then refuses the linked claim as `unverified` when its selected stdout is unavailable, digest-mismatched, or its mechanical `axis`/`author`/`result`/`findings` fields disagree. Genuinely external evidence omits that worker provenance. Prior denials and inert records remain separate fields. Stale evidence never satisfies current obligations.
