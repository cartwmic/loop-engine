...
- Intent acceptance is a closed criterion spine: each current record is `{id, statement}` with a stable run-local `AC-N` ID. Design, plan, implementation, and validation may reference those IDs where useful; references are optional and are checked only for shape, local duplicates, and current-intent membership.
...
The three shipped profiles use the same run-local criterion spine. `intent.json.acceptance` contains closed `{id, statement}` records whose IDs match `^AC-[1-9][0-9]*$`; the trusted driver preserves an ID while its meaning is materially unchanged. Optional `criterion_id`/`criterion_ids` fields on design coverage, plan tasks, implementation validation rows, and validation proof rows reference only the current intent. References are not a complete trace matrix: the provider checks syntax, local duplicate-freeness, and current-intent membership, while reviewers judge semantic coverage.
...
For checked transitions, evaluation performs deterministic schema and link checks before consulting review evidence or the finding ledger. Missing or unparseable expected artifacts produce a schema denial; invalid or inaccessible artifact roots produce an evaluation error. Ledger denials identify missing, malformed, stale, source-unlinked, or set-inconsistent driver snapshots; evidence denials identify unsatisfied policy axes. Check-free `revise` transitions do not require provider evaluation. Every `finding-ledger` snapshot remains an ordinary immutable context record, so `show` exposes the current snapshot and its full history without chat state. Review packets carry that ledger context and the frozen worker assignment identifies one axis; reviewers inspect only current entries whose `review_axes` contains that axis, without changing policy or verdict authority. Finding sources use only `context-record` references; the provider resolves the referenced evidence and derives current checkpoint identity for report subjects. The implementation slot forwards ledger, review-evidence, and applicability context to `run-plan-graph`, so provider-side source references resolve without driver copies; the runner adds only exact-task `finding_context` entries for current accepted unresolved implementation-owned findings. Inner task stdin remains exactly `{artifact_root, task}`. Omitted invocation input executes the full DAG. Focused bound re-invocation uses `loop-engine invoke RUN_ID implement --input '{"plan_revision":"REVISION","task_roots":["TASK_ID"]}'`; the provider validates the closed shape and current revision, executes unique known roots plus dependants, and requires same-revision standing results for prerequisites outside that selection. Proposal records never enter gates or packet construction. The provider-side validation API is `finding_ledger::evaluate_finding_ledger(context, gate, subject, current_revision, config, failing_evidence)`; its `current_snapshot()` projection and routing projection are mechanical input and never make a semantic decision.
