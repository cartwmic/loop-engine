...
- R1. The engine PRD's 9-state reference topology remains authoritative. Review states preserve nearest `revise` edges and add only explicit check-free owning-phase routes: `design-review → explore`, `plan-review → design|explore`, and `implementation-review`/`validation → plan|design|explore`; no new states or dynamic targets.
...
- R21. The shipped configs embed example prompts ported from the v1 rubric set for every axis in R12's table, plus `reviewer-protocol.md` defining binary evidence, mandatory failure burden, candidate triage before append or mutation, scope/materiality classification, focused external reconsideration, comprehensive and confirmation rounds, late-finding proof, three-round circuit-breaker escalation, no-waiver semantics, and untrusted-material handling. Late-finding proof requires current evidence, violated in-scope obligation, concrete consequence, validation gap, and provenance classifying the issue as newly exposed, fix-introduced, or previously overlooked; prior visibility or reviewer overlook does not waive a known material defect. Together these are the shipped reviewer contract; each config file carries a version, and evidence names it (R13). Orchestrators may adapt or replace prompts (R11); the A11 calibration obligation applies to the shipped defaults only.
...
- R23. Authoring-state instructions are static text pointing at the shipped templates, frozen config, and reviewer protocol; per-run obligations are read from `show` (initial input), never generated dynamically. Review guidance tells operators to triage candidate reviewer output before append or mutation, seek focused external reconsideration for disputed candidates, run comprehensive first review and bounded confirmation review, prove late findings with the full evidence/consequence/validation/provenance burden, and select the phase-named owning route. The static authoring guidance carries: intent — problem, outcome, acceptance boundary, constraints, non-goals, and target-laundering warning; design — structural shape, not a work schedule; plan — task-packet dependency graph with contract gates before parallel fan-out; implement — doc integration is part of the change.
...
- No new states, dynamic targets, or checkpoint state (D9 deferred; checkpoint axes ride implementation review). Explicit phase-named check-free owning routes in R1 are part of this reference topology; no `revise` event is removed.
...
- A14. `describe` output matches the engine PRD's reference software-change topology exactly, including nearest `revise` and phase-named owning routes (scripted comparison).
