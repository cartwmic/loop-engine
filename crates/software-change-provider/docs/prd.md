...
Amended 2026-08-12 (explicit amendment, owner-approved): the v0.1 repo-local-only distribution statement is superseded for v0.2.0 by standalone distribution: release archives provide the `loop-engine` and `software-change` binaries, and `software-change` embeds its shipped data and exposes `data-dump` to materialize the repository-relative data tree. Repository checkout remains a supported development path. The existing stdin/stdout `describe`/`evaluate` wire contract is unchanged; no workflow, gate, policy, or other requirement changes are made.
