...
fn invoke_overlay_overrun_is_not_already_running() {
    ...
    let mut live_command = Command::new("sleep");
    live_command
        .arg("60")
        .stdout(Stdio::null())
        .stderr(Stdio::null());
    super::bounded_process::prepare_process_group(&mut live_command);
    let mut live = live_command.spawn().expect("spawn live pid");
...
fn invoke_worker_ppid_equals_waiter_pid_not_invoke() {
    ...
    let child = command.spawn().expect("spawn invoke");
    let invoke_pid = child.id();
    let output = super::bounded_process::wait_existing(child, "loop-engine invoke")
