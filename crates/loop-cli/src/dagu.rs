...
/// Isolated home directory: `capture_root/dagu-home`.
pub fn isolated_home(capture_root: &Path) -> PathBuf {
    capture_root.join(HOME_DIR)
}

/// Locator path: `capture_root/dagu-locator.json`.
pub fn locator_path(capture_root: &Path) -> PathBuf {
    capture_root.join(LOCATOR_FILE)
}

/// Unique `dag_name` and `run_name` for this capture root: `fanout-<dir-name>`
/// when that fits Dagu's 39-character graph-name limit; otherwise a stable
/// `fanout-<hex>` digest of the absolute capture path so bound invocation
/// directories remain unique and valid.
pub fn names_for_capture_root(capture_root: &Path) -> Result<(String, String), DaguError> {
    let suffix = capture_root
        .file_name()
        .and_then(|name| name.to_str())
        .filter(|name| !name.is_empty() && *name != "." && *name != "..")
        .ok_or_else(|| DaguError::Locator {
            message: "capture_root must have a non-empty directory name for dagu locator names"
                .to_owned(),
        })?;
    let name = dagu_graph_name(capture_root, suffix)?;
    Ok((name.clone(), name))
}

fn dagu_graph_name(capture_root: &Path, suffix: &str) -> Result<String, DaguError> {
    let candidate = format!("{NAME_PREFIX}-{suffix}");
    if is_dagu_graph_name(&candidate) {
        return Ok(candidate);
    }
    let identity = fs::canonicalize(capture_root).unwrap_or_else(|_| {
        if capture_root.is_absolute() {
            capture_root.to_path_buf()
        } else {
            env::current_dir()
                .map(|cwd| cwd.join(capture_root))
                .unwrap_or_else(|_| capture_root.to_path_buf())
        }
    });
...
/// Write `capture_root/dagu-locator.json` and create `capture_root/dagu-home/`.
///
...
pub fn write_locator(
    ...
    let capture_root = absolute(capture_root)?;
    let home = isolated_home(&capture_root);
    ...
    fs::write(locator_path(&capture_root), encoded)?;
    Ok(locator)
}
