...
pub enum RolePayload {
    ...
    ValidateInputs(ValidateInputsPayloadDto),
    EvaluateGates(GatePayloadDto),
    LiveGuidance(GuidancePayloadDto),
    CheckCompatibility(CompatibilityPayloadDto),
}
...
#[serde(untagged)]
pub enum AnyResult {
    Describe(DescribeResultDto),
    ValidateInputs(ValidateInputsResultDto),
    EvaluateGates(GateResultDto),
    LiveGuidance(GuidanceResultDto),
    CheckCompatibility(CompatibilityResultDto),
}
...
pub fn result_envelope(
    ...
    let result_value = match result {
        AnyResult::Describe(result) => serde_json::to_value(result).unwrap(),
        AnyResult::ValidateInputs(result) => serde_json::to_value(result).unwrap(),
        AnyResult::EvaluateGates(result) => serde_json::to_value(result).unwrap(),
        AnyResult::LiveGuidance(result) => serde_json::to_value(result).unwrap(),
        AnyResult::CheckCompatibility(result) => serde_json::to_value(result).unwrap(),
    };
    serde_json::json!({
        "protocol_major": PROTOCOL_MAJOR_V1,
        "role": role,
        "invocation_id": invocation_id,
        "provider_version": "scenario-provider/0.1.0",
        "result": result_value,
    })
}
