...
For late findings, follow the provider skill's proportional guide: keep validation-report-only corrections in validation; use `revise-implementation` (and existing plan-task selection) for implementation defects; use `revise-plan`, `revise-design`, or `revise-intent` only when that owning phase's obligation is materially wrong. Reconfirm only the affected downstream proof and preserve valid work; deeper backtracking is exceptional. Captured ad hoc repair and semantic owner override are not shipped, and `override-carry` is mechanical provenance only, not a bypass or waiver. Human-facing challenge review must still require current evidence, meaningful falsification, a violated frozen obligation, and a concrete consequence; unchanged `*-adversarial-review` IDs remain machine identifiers.
