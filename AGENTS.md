...
Use `amend-binding` only for owner-attested future execution corrections, never frozen policy/topology or past attempts. Ordinary invoke preview/selection/controls/timeout do not require an amendment. Overrun never authorizes retry or departure while owned work lives: wait or `cancel-invocation`, verify cleanup, then observe again. Cancellation's ten-second clock applies per controller acquisition/resumption (at most three seconds graceful), not across an interrupted controller's unbounded operator-delay gap. Later admission stays blocked in that gap; already-running work may persist. Historical missing ownership is unsupported, not permission for arbitrary-PID cleanup. Keep fixed original providers for older semantic profiles; no active bootstrap migration or reinterpretation is authorized.
