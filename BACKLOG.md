...
The 0.22.0 notes give concrete checks: `setup` could not assemble an owner-approved one-author custom profile; `preview-bindings` redacted preambles and could not serve as a start file; an already wrapped implement binding became double-wrapped; a renamed `config_version` suffix silently changed semantic coverage. Inspect effective roster/floor and model diversity as owner-visible trade-offs, not a universal second-model requirement or permission to mutate a frozen run's policy.
