...
10. **Stabilize the requirement catalog, then audit semantic proof.** First run [Separate enduring product requirements from implementation specifications](#separate-enduring-product-requirements-from-implementation-specifications) as its own requirements-authority change. After the resulting IDs and requirement clauses are accepted, run [Audit semantic Bookends proof and retain the matrix](#audit-semantic-bookends-proof-and-retain-the-matrix) separately against that stable revision. The audit must map every live requirement and conjunctive clause to its public scenario, call path, observable assertion, and material proof verdict; retain the revision-bound matrix and give every gap an explicit repair, citation correction, or accepted-gap disposition. Do not build the exhaustive matrix before restructuring the catalog only to invalidate it immediately.
11. **Keep additional public-proof corrections in a parallel lane.** Run [Reject citations outside approved Bookends surfaces](#reject-citations-outside-approved-bookends-surfaces), [Reconcile LE-38 requirement wording and public proof](#reconcile-le-38-requirement-wording-and-public-proof), and [Add public contract tests with Bookends IDs](#add-public-contract-tests-with-bookends-ids) separately as their product decisions become ready. They do not replace the exhaustive semantic audit and do not belong in provider-repair runs.
12. **Leave new workflow initiatives until operating evidence justifies them.** [Generalize provider evaluation and calibration](#generalize-provider-evaluation-and-calibration) and [Create a prototype-to-spec workflow](#create-a-prototype-to-spec-workflow) are later standalone initiatives, not cleanup to bundle into the packages above.
