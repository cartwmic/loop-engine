...
Separate 0.22.0 dogfooding found that Bookends prerequisites can become a validation-time surprise: `bookends.toml`, live requirements with eligible public citations, required CI collection and the evaluate process's repository cwd. Investigate an early, read-only prerequisite check or clearer setup/plan guidance; do not weaken the final gate.

## Other wanted improvements

Observations marked as separate 0.22.0 dogfooding below came from another machine; their captures are not in this checkout. Recheck source evidence or reproduce current behavior before changing product contracts.
...
Status (2026-09-20): substantially delivered by run-1789764810283193000-1-10500 (commit 38f89d1, completed with zero overrides): status/visibility lanes separate execution, conformance, acceptance, evidence, freshness and uncertainty; acceptance projects explicit attributed decisions; passive owner updates per LE-143. AC-1..AC-5 pass with executed proof. Subsequent dogfooding identified status readability, read-output duplication and observation-use concerns below; product-level review-commission pruning also remains.

A separate 0.22.0 run reported empty `inner_workers` while a bound invocation was live; the driver relied on `ps` and cancelled useful work. The same notes describe a preemptive manual move of an engine-owned `ownership/` directory before retry. Test progress and ownership on live, partially finished invocations and teach the driver to trust recorded ownership/captures, not process-list guesses or manual state edits. Establish whether stale ownership still blocks recovery before proposing a cleanup command. `inner_workers` is documented as empty while running and `invocation-progress`/`monitor` already report per-step state, so first check whether the driver guidance points to them and whether step state maps clearly to assignment IDs and attempts.

### Make human status output digestible on active runs

Owner-observed pain (2026-09-21, second machine, brand-new active run): human `show --view status` renders lines and lines of wrapped text instead of the ~8-line digest demonstrated on the completed run-1789764810283193000-1-10500. Root cause, read from `render_show_compact` (`crates/loop-cli/src/lib.rs`): the human status output is the 8 digest lines **plus the entire status JSON packet appended inline** as one single-line blob (`status details ...`). On a finished run that packet is ~2 KB; on an active run it can carry current-state latest evaluations with feedback text, visibility lanes with reason prose, active invocation ownership/selection objects, and requestable-event details — so the terminal can wrap one mega-line into pages and the digest scrolls away unseen. Related: `--view full --json` measured 9 GB on the completed run (~260k duplicated assignment/ledger-adjacent records); that view is machine-only and must never reach a terminal. Human status attempts invocation-progress collection when an invocation is selected; measure when this triggers live Dagu work and its cost.
...
Directive: investigate first for the correct shape of human status — what an owner needs at a glance (current work, next action, blocker, evidence location, freshness, uncertainty) versus what belongs behind locators — against a live messy mid-implementation run, not a finished one. Fix what fails that shape; do not add a parallel monitoring system. Any CLI change keeps status non-arming and preserves the applicable show output and exit-status contracts.

Owner-observed compliance signal (2026-09-22, other runs): agents pipe `show` output to null between transitions instead of reading it. Note the sharp edge: arming happens server-side, so `show ... > /dev/null` satisfies the observation interlock's letter (visit armed, mutation allowed) while defeating its purpose (acting informed). Output cost may contribute to this behavior, but the cause is unconfirmed. Investigate alongside duplication and byte cost: quantify what an agent needs from action/full per transition; consider machine-first shapes (digest plus locators by default, bulk on demand); and determine whether interlock compliance should mean anything beyond a server-side arm. Address the cause rather than assuming a punitive gate will make agents read the output.

Extend the investigation to every read primitive agents actually consume — show views (status/action/full, human and JSON), history, invocation-progress, monitor packets, list, and the instruction/guidance surfaces — and audit for duplicated information across them: the same evaluation text, lane prose, ledger content, or invocation detail repeated in multiple outputs or repeated per record. Machine consumers stay complete (agents read this output; nothing they need is removed), but each fact should have one authoritative home with locators elsewhere, so agents stop paying context-window rent on the same bytes N times. Demonstrate byte counts per primitive on a live run before and after; do not merely assert efficiency. A separate 0.22.0 run reported 211 context records / 18.8 MB, a 47 MB fan-out spec and `show` taking over 100 seconds; repeated full-intent steering and per-worker context copying are leads to measure, not established causes here.
...
Review slots commission every record of their admitted kinds (observed: 511 records / ~1MB to an implementation-review worker, mostly other gates' ledgers). Excess context can break small-window reviews while remaining less visible with larger models. Run-1789764810283193000-1-10500 proved current-gate-plus-references filtering (511 to ~125 records, preamble byte-identical, all reviews green) via run-scoped binding wrappers only — the product default is unchanged. Scope a provider change that commissions review-necessary records while preserving reuse/applicability references, LE-127 meaningful context, and full verification. Add a journey assertion on commissioned review-context size plus a small-window review probe so the gap cannot hide again. Another 0.22.0 run reported aggregate/challenge workers receiving earlier passes despite a preamble saying they would not; inspect the delivered context and fix the filter or wording without losing necessary reuse evidence.

### Make review passes grounded and challengeable

Separate 0.22.0 dogfooding reported 84/84 passes from one intent-review model while a review with the owner's original request found gaps; pass rationale was often retained in raw stdout but omitted from evidence triage, and a wrong run citation survived both gates. Investigate compact, checkable reasons for passes, driver verification of cited text/IDs and necessary repository reads, and challenges of the parent's actual reasoning rather than an empty pass. Check the effect of aggregate-only axes, prompt wording and resolvable version-correct templates. Criterion and goal verdicts need the same checkable reason as axis passes. A later run with a second review model caught intent defects the first model passed at both gates; use such cases as validation data. Avoid boilerplate, duplicated evidence and a blanket mandate for more workers; validate on real misses and sound passes.

### Preserve owner intent and truthful authorship

When the driver drafts intent, review it against the owner's actual request before approval and again after material intent revision; downstream fidelity to the approved intent cannot recover an omission at that boundary. Attribute driver-written artifacts, steering and finding decisions to the driver, not to a human who approved a summary. Investigate the smallest repeatable drafting/approval check, including defined pass/fail terms and per-outcome requirement classification, without assuming a new provider axis or sending private conversation context to every worker.
...
Investigate actual failed captures across review and implementation, including which fields fail, whether the original meaning is recoverable, and the cost and outcome of the existing retry. Compare small remedies before choosing one: clearer or simpler schemas/prompts (one lead: the review preamble's "Return the frozen `review_stage` constant together with {…}" preceded repeated outputs that put a bare `review_stage` line inside the JSON fence or omitted the field), constrained structured-output support where available, deterministic syntax repair only where it cannot invent content, or an explicitly attributed conversion of the retained raw attempt by the same worker, a stronger model, or the driver. Test whether a failed retry can be repaired without repeating the whole substantive review, while keeping the original attempts immutable and linked to any conversion. Never silently infer missing decisions, promote unverified content to accepted evidence, change reviewer authority, or claim schema conformance from semantic plausibility. Unrecoverable or conflicting content remains unusable, with an intelligible escalation path. Prove the chosen approach on real malformed-but-useful outputs and on negative cases that must still fail closed. A separate 0.22.0 run reported one malformed worker exhausting correction and forcing a full fan-out rerun despite other selected outputs. Check surgical `--assignment` recovery and whether failed-invocation successes remain citable through the provider gate; cancelled invocations reportedly lose finished-output linkage. Preserve immutable attempt and invocation identity, and do not claim that subset retry is valid solely because `invoke` accepted it.

### Document and prove surgical reruns of fan-out gates

Separate 0.22.0 dogfooding reran whole review gates for one worker's format failure or one slow worker, costing roughly 10–20 minutes each time, although `invoke --assignment`/`--assignments` exists. A subset rerun after a barrier failure kept group order and assignment IDs, and evidence could cite selected outputs of the failed invocation; capture directories were renumbered by selection position, so directory names no longer matched assignment IDs. Separately, after `cancel-invocation`, evidence citing that invocation's already finished workers was refused (`selected-output-linkage-refused`), so cancelling to recover one hung worker discarded every other finished judgment. State the supported recovery path in agent usage and the provider skill (which assignments to select after a barrier failure, overrun or cancellation; read identities from the fan-out spec, not directory names), establish whether refusing linkage after cancellation is intended and, if finished conforming assignments can be kept safely, keep them selectable with their cancelled origin visible. Prove a gate reaches an accepted outcome from evidence mixed across invocations. Do not add automatic retries unless the documented path proves insufficient, and do not relabel cancelled work as completed.

### Reject reviewer reuse rows that name no delivered applicability record

A 0.22.0 challenge worker returned `{"reuse": <review-evidence ID from another gate>}` instead of a judgment. Reuse is valid only through an applicability record already in the commissioned context. Establish whether conformance or candidate expansion rejects this mechanically before driver triage, with a diagnostic naming the rule; if not, reject it there. Consider not offering the reuse row to workers whose context holds no applicability records. Never treat an invalid reuse row as a pass or a carry.

### Design per-criterion proof before validation

Two reported runs passed plan review with proof that could stay green despite an undelivered behavior; later validation cited the same command set for every acceptance criterion, and challenge review found uncovered outcomes. At plan time, name for each criterion the observation that would distinguish success from failure. At validation, inspect what each cited command actually asserts and retain only relevant evidence per criterion; valid capture IDs or passing scripts alone do not establish coverage. Use existing plan/report/evidence paths rather than a parallel criterion ledger.

### Disposition material findings on their merits

A completed run's later challenge gates reportedly produced findings whose rejection reasons conceded defects but cited the cost of reopening work; ledger authorship sometimes implied a human decision the driver made. A parent pass and re-entry cost do not refute a finding. Require evidence-based merits dispositions and truthful authorship; route a real but expensive defect to the owner through existing exceptional authority rather than relabel it rejected. Coordinate with the repeated-work item above to reduce avoidable re-entry, without waiving unresolved material defects.

### Make review-evidence recording deterministic and fail-closed

A dogfood helper hardcoded `pass` while writing records derived from reviewer outputs; that happened to match its all-pass sample but would misrecord a fail. Investigate a small shared helper or provider command that reads selected attempts, retains actual result/findings and origin across gate layouts, refuses missing selections, and presents candidate review-evidence and validation-verdict records for driver triage before append. Finding-ledger dispositions remain driver-authored; do not auto-append or treat process/schema conformance as semantic approval.
...
The 0.22.0 notes give concrete checks: `setup` could not assemble an owner-approved one-author custom profile; `preview-bindings` redacted preambles and could not serve as a start file; an already wrapped implement binding became double-wrapped; a renamed `config_version` suffix silently changed semantic coverage. The double-wrap and the `config_version`-suffix switch are small defects that can be fixed on their own, ahead of any layering design. Inspect effective roster/floor and model diversity as owner-visible trade-offs, not a universal second-model requirement or permission to weaken a frozen run's policy. A separate run added a second review author mid-run by binding amendment; amendments do not change author counts, so the provider still required one passing author per axis and the driver enforced two by hand, and the amended aggregate author received criterion author number 2 with no prechosen verdict IDs. Decide with the owner whether an attested amendment that only raises floors is acceptable, or whether restart remains the answer; if restart, say so in the skill. (Speculative: one occurrence.)
...
- **Early worker termination after a failed invocation:** If a newly invoked worker is again killed (`signal: terminated`) seconds after start, before its model runs, and shortly after a failed or cancelled invocation on the same slot, retain both invocations' ownership records, captures and timestamps and establish whether cleanup of the earlier invocation reached the new one. Do not assume an immediate successful rerun explains it.
- **Older software-change/CLI dogfood leads:** Defer minor `show`/`@PATH`/payload-less `event` diagnostics unless repeated ordinary use makes them material. Recheck the earlier `proof_id` documentation complaint and contract-v2 `run-validation` refusal against current supported contracts before treating either as an active defect; 0.22.0 `run-validation` emits `proof_id` in command-evidence candidates and its recovery contract accepts contract versions 2 and 3, so only template/schema documentation of `proof_id` for hand-written reports may remain.
