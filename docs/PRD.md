...
Bound mode reads the existing, unchanged five-key invoke packet (including `capture_dir`) and rejects `--instructions`. A worker without `preamble` receives the byte-identical legacy payload, including when it declares only `output_schema`. A worker with `preamble` receives, in exact order: the decoded preamble bytes unchanged; exactly one appended LF only when those bytes do not already end in LF; compact JSON serialized from exactly `{"artifact_root": packet.artifact_root}`; one LF; literal `---\n\n`; then the unchanged legacy base payload (existing instruction-body LF normalization, blank line, and `run_id`/`slot_id`/resolved `artifact_root` trailer). The new context block contains no `capture_dir` or duplicate run/slot identity. The invoke packet, digest input, and gate matching do not change. Ad hoc mode has no invoke packet: with `preamble` it emits the preamble, the same conditional LF, literal `---\n\n`, then unchanged instruction-file bytes, with no fabricated artifact-root context; without `preamble`, instruction bytes remain byte-identical.

Bound mode honors `packet.capture_dir`: it writes per-worker stdout/stderr under `0/`, `1/`, … plus `summary.json` in argv order. For a worker with `output_schema`, fan-out accepts stdout only when the entire whitespace-trimmed output is a JSON object or when otherwise arbitrary prose contains exactly one fenced `json` block whose content is a JSON object. Missing, malformed, non-object, or ambiguous candidates fail conformance. Fan-out checks only presence of the declared top-level keys and does not interpret names or values. Every worker summary entry contains `command`, `args`, the true process `exit_code`, `stdout_path`, and `stderr_path`. A worker with `output_schema` additionally receives `status` (`succeeded` or `failed`) and receives `conformance_error` only on failure. Both new fields are omitted for workers without `output_schema`, preserving the legacy summary shape. Any conformance failure is a collector failure, but `summary.json` is written first so the driver can inspect it and captured stdout before stderr. Ordinary inner nonzero exits alone remain recorded and do not fail the collector. Exit 0 and mechanical key presence do not establish semantic deliverable validity.

The command starts one process per worker in parallel, reaps every worker it spawned before exit, and on collector success prints a JSON summary that is not a run-state envelope and not a provider evidence schema. It encodes no harness.
...
- Nested fan-out workers accept only `command`, `args`, optional opaque `preamble`, and optional `output_schema` with exact required-key syntax. Opted-in bound stdin adds only the compact one-key `artifact_root` context and fixed separator ahead of the unchanged legacy body/trailer; unopted stdin, the five-key packet, digest, and gate matching remain unchanged. Ad hoc framing adds no run context. Contracted output records preserve process exit and capture paths, add conformance status/error, and are written to `summary.json` before collector failure.
