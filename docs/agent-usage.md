...
New supporting profiles declare `contract_version: 3` and use `minimal-10`, `standard-10`, or `high-rigor-10`. Criterion and whole-goal author floors are independently 1/2/2 for minimal/standard/high. All levels carry ordinary and challenge review at every phase; minimal uses one aggregate all-axis author, standard uses two aggregate all-axis authors, and high uses the same two identities for individual-axis review followed by fresh aggregate review on unchanged work. Shipped profiles remain unbound and Bookends-off by default. The provider rejects v2 semantic evaluation in new binaries; keep the fixed original provider/runtime for v2 runs. Provider-free show/history preserve old input, topology, ownership and evidence meaning without migration.
