...
Use `fan-out` **ad hoc** when you want parallel worker CLIs without a run: pass `--instructions FILE` and do not send an invoke packet on stdin. Without `preamble`, each worker receives byte-identical instruction-file bytes. With `preamble`, stdin is the preamble bytes, one LF appended only if absent, literal `---\n\n`, then the unchanged instruction-file bytes. Ad hoc mode has no invoke packet and fabricates no artifact-root context. Zero `--worker` entries fail closed.

When a work slot is frozen to `loop-engine` args that begin with `fan-out`, the legal start remains `loop-engine invoke RUN_ID SLOT_ID`. Do not call `fan-out` yourself for that slot; `invoke` execs the frozen argv with the existing worker packet on stdin. Bound mode rejects `--instructions` and keeps that packet at exactly `run_id`, `slot_id`, `artifact_root`, `instruction_body`, and `capture_dir`. A worker without `preamble`, including an `output_schema`-only worker, receives the byte-identical legacy payload. A worker with `preamble` receives, exactly: preamble bytes; one LF only if absent; compact one-key JSON `{"artifact_root": packet.artifact_root}`; one LF; literal `---\n\n`; then the unchanged legacy instruction-body/newline/trailer payload. The new block has no `capture_dir` or duplicate run/slot identity.

Bound mode honors `packet.capture_dir` (writes per-worker `0/`, `1/`, … plus `summary.json` there). For `output_schema`, stdout must be either a bare JSON object or the sole fenced `json` object amid prose; malformed, missing, non-object, and ambiguous output fails. Fan-out checks only declared top-level key presence. Every summary entry has `command`, `args`, true process `exit_code`, `stdout_path`, and `stderr_path`; contracted entries add `status` (`succeeded` or `failed`) and failure-only `conformance_error`, while uncontracted entries omit both new fields. Fan-out writes `summary.json` before returning collector failure for any nonconformance. Read that summary and captured stdout before stderr. Ordinary worker nonzero exits retain the existing collector treatment. Exit 0 and key presence do not establish semantic deliverable validity.
