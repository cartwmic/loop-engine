...
Use each provider's construction procedure. For software-change, use the public `software-change setup` command described below: it reads the selected embedded profile, assigns each policy to its first N roster entries, groups aggregate-stage policies by author, and emits singleton individual-stage assignments. Research and policy-document retain their provider-specific per-policy construction. Ordinary/challenge gates stay separate. The software-change roster has closed `{author,command,args}` entries; it does not choose a model.

Software-change setup writes only its `--output` path after input and `preview-bindings` validation, displays the exact generated bytes and SHA-256, and does not start or wait for a run. Recheck that hash immediately before `start` and start only the unchanged file; no post-preview merge is allowed. The frozen assignment is authoritative for review workers. Later state instructions are context only for them, not commands to perform driver work.
...
`fan-out` does not start, advance, or record a run and does not open the run database. Each invocation emits a local Dagu `type:graph` only under an isolated `capture_dir/dagu-home/` (bound: `packet.capture_dir`; ad-hoc: `cwd/fan-out-adhoc/<unique>`). Callers never supply Dagu YAML. The facade waitpids `dagu start --quiet --dagu-home` and does not daemonize. Drivers poll `show` for overlay; while overlay is `running`, poll `invocation-progress` for per-step graph state and named traces. Overlay remains the facade process exit; Dagu is not a Loop Engine operation. Dagu is GPLv3: invoke the operator-provided binary as a subprocess only; do not embed its Go API. Packages do not ship `dagu` (minimum 2.14.0 on PATH). An optional single `--then` divider creates two flat worker groups: selected second-group workers depend on selected first-group completion and mechanical conformance, while semantic output is opaque and a conforming semantic failure does not block the second group. Malformed, empty, or repeated divider shapes fail before any worker starts.

```text
loop-engine fan-out [--worker JSON]... [--then --worker JSON ...] [--instructions FILE] [--max-active N]
```

Workers come only from repeated strict `--worker` JSON objects. Each requires string `command` and array-of-string `args`; command and argv values containing LF or CR are rejected before Dagu setup because Dagu command text cannot contain a line break. A worker may also contain string `preamble`, legacy `output_schema` with exactly `{"required":["key", ...]}`, and additive `full_output_schema` containing a complete JSON Schema (the explicit `{schema, retry_limit: 1}` wrapper is also accepted). Unknown and malformed fields fail closed. `output_schema` remains required-key presence only for compatibility; `full_output_schema` validates the extracted JSON candidate against every declared JSON Schema constraint and gives the same worker one correction attempt, never a replacement worker. Its optional `x-loop-engine-force-fresh` annotation is an additional opaque JSON Schema constraint: bound `controls.force_fresh: true` adds it to `allOf` before launch, validates it and captures that effective schema without rewriting the binding. Software-change declares fresh-row requirements there so a reuse row cannot satisfy a force-fresh assignment. Other reference checks remain provider-owned; the engine does not learn review semantics. These are nested fan-out fields only: outer bindings accept `{command,args,context_filter?}`; nested task workers remain `{command,args}`.
...
Shipped profiles omit `work_slot_bindings` (or `{}`), so slots stay driver-performed until opt-in. Review Pi workers keep `--no-skills --no-extensions`, add an existing `-e` path for each extension required by the selected model provider, include `--tools read,grep,find,ls`, name their model, and must not pass `--no-context-files`; omitted paths produce no `-e` pair. Implement workers do not add `--tools`. `preview-bindings` warns when a pi worker has `--no-extensions` and no `-e`; missing `--no-extensions` is not a required warning.
...
The pre-push hook passes Git's real four-field update stream to
`bookends-check --updates-stdin`; required push-to-main CI passes the event's
actual before/after/ref identities. Publication mode enumerates every commit
reachable from each new tip but not its old tip, including all merge parents,
and reads config, PRD, workflow, and proof bytes from those immutable trees.
A zero old tip means all reachable history, with commits lacking the
historical enabling config treated as pre-adoption. Missing ancestry, failed
ordinary fetch, interrupted/limited enumeration, or a changed local
HEAD/branch/index/worktree
is incomplete and cannot be GREEN. Each publication invocation writes a
write-once range/completeness receipt outside the checkout; bypasses retain
their separate durable reason record.
...
## Software-change continuation and compatibility

New supporting profiles declare `contract_version: 3` and use `minimal-11`, `standard-11`, or `high-rigor-11`. Criterion and whole-goal author floors are independently 1/2/2 for minimal/standard/high. All levels carry ordinary and challenge review at every phase. Intent review adds the distinct `acceptance-granularity` and `owner-comprehensible` questions; minimal and standard review all intent axes at their aggregate stage, while high keeps its five existing intent axes at individual and aggregate stages and reviews the two added questions at aggregate stage. Existing stages and independent-author floors remain unchanged. Shipped profiles remain unbound and Bookends-off by default. New v11 graphs add the provider-owned reconciliation state after implementation editing and before final implementation proof/review; v10 and older stored runs preserve their original topology and evidence. The provider rejects v2 semantic evaluation in new binaries; keep the fixed original provider/runtime for v2 runs. Provider-free show/history preserve old input, topology, ownership and evidence meaning without migration.

Use the provider's deterministic setup command for new per-run bindings:
...
The roster is an ordered closed array of `{author,command,args}` records. Setup reads embedded profile, preamble and output-schema data, assigns the first required authors for every stage, emits high-rigor individual/aggregate review groups behind the generic `fan-out --then` barrier, preserves an optional closed `{command,args}` `--draft-worker` as the bound `intent-draft` worker, and may emit an implementation binding. It validates the output contract and runs `preview-bindings` before atomically writing the generated profile. It does not start a run, choose a model, adapt a CLI, save preferences, or manage a worktree. Caller-supplied model/effort arguments remain opaque roster/draft-worker arguments. Confirm the exact generated bytes, hash, bindings, stages, author assignments, filters and concurrency, then rehash immediately before `start`; changing any byte requires a new confirmation.

Every new contract-v3 plan task has a non-empty unique `criterion_ids` list of current AC-N IDs. Intermediate links remain optional. Final validation uses one retained command collection and a fixed complete AC/goal index. Individual high-validation workers return axes only; aggregate workers alone return criterion/goal rows. Accepted-unresolved findings remain blocking across revisions, while unaffected review reuse requires an explicit evidence-applicability record.

### New-run reconciliation and proof ordering

For a new v11 software-change run, `implementation-ready` enters the provider-owned `reconciliation` state before implementation review or the final implementation proof boundary. Its unbound `reconciliation-draft` slot authors `reconciliation.json`; `reconciliation-ready` is the checked handoff. The state does not write reports or checkpoints and does not commit. It must make one honest branch decision: sufficient existing wording needs only implementation and public proof; change-specific proof creates no requirement proposal or PRD commit; an implementation defect requires a recorded code correction; only missing or changed enduring meaning requires exact owner acceptance, separately authorized application and commit, updated traceability, and independent inspection. A justified no-document-change result is valid, while a blocked result names concrete blockers.
...
Use the software-change skill and shipped validation template for full record forms. The `data/recovery-contract-v2.json` fixture remains a compatibility test/example for the generic v2 parser; it is not the shipped v3 profile universe. Policy-document and research gain the common engine controls, not software-change-specific steering, disposition, grouped review or criterion contracts. Simple-first/YAGNI/KISS apply to all providers; meaningful current need and inadequate simpler alternatives belong in the ordinary design, not a new justification framework.
