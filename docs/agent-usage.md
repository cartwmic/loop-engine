...
Reviewer output is candidate evidence. Preserve each raw response and append a driver-authored `finding-ledger` snapshot after triage. An `advisory-finding-proposal` is only a suggestion: edit or reject it as needed. It never satisfies a gate or changes a task packet. Only current driver-accepted unresolved implementation findings with an exact task route appear as that task's `finding_context`. Omitted invocation input runs the full plan; a focused same-slot invocation runs the selected roots plus dependants after the standing-prerequisite checks described below. `show` is the fresh-actor view of the immutable ledger history and proposal records.
