...
>
> **Checkpoint B evidence:** `checkpoint_b` production E2Es prove provider conformance plus active-run compatibility; missing-executable, timeout, crash, nonzero, signal, malformed, extra/missing output, wrong-major, invalid-UTF-8, oversized-stdout, and bounded oversized-stderr behavior; zero/valid/rejected/evaluation-error inputs; strict duplicate-key rejection before provider execution; executable drift between description and validation; no run/journal writes on rejected or failed creation; atomic persisted run creation; fresh-process lifecycle-filtered listing and authenticated continuation; provider-free termination after registration tombstoning; repeat-terminal rejection with immutable journal evidence; complete persisted history rendering; and correlated provider/persistence traces. Closed manifests are `quality/facets/v1/provider.check.json`, `quality/facets/v1/run.create.json`, `quality/facets/v1/run.list.json`, `quality/facets/v1/run.terminate.json`, and `quality/facets/v1/run.history.json`.

> **Checkpoint C evidence:** `checkpoint_c` production E2Es prove authoritative active, neutral-final, initial-final, intentional zero-final ongoing, non-final sink, and terminated projections; immutable inputs, graph revision, stored static guidance, live-guidance capability, empty caller-owned evidence selection, requestable targets and required gates; terminal/sink empty-event semantics; fresh-process operation under tombstoned provider configuration with an empty provider ledger; not-found taxonomy; human output; and correlated persistence traces. Closed manifest is `quality/facets/v1/run.show.json`.
>
> **Checkpoint D evidence:** `checkpoint_d` production E2Es prove gate-free and gated completion, pass/fail/mixed/exact-set/incompatibility/evaluation outcomes, strict inline evidence, unknown/final/terminated rejection, self-loop/cycle semantics, gate-free continuity under a tombstoned registration, gated tombstone failure, stale-CAS post-transaction authority, representative malformed/timeout provider failures, exact persisted request notes and transition facts, human/JSON delivery, and correlated provider/persistence traces. Selected/provider evidence and updated/restored-configuration rows remain deferred to WP6. Closed manifest is `quality/facets/v1/run.request.json`.
