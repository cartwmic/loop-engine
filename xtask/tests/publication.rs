...
use xtask::publication::{ParsedUpdateDisposition, parse_updates, run_publication};
use xtask::report::{
    DerivedDisposition, GateDecision, RejectionCode, Store, UpdateKind, sha256_hex,
};
...
fn rejected_and_deletion_attempts_have_exact_nullability_and_only_common_dir_git_query() {
    ...
    assert_eq!(trace.matches("\"event\":\"start\"").count(), 2, "{trace}");
    assert_eq!(trace.matches("--git-common-dir").count(), 2, "{trace}");
    for forbidden in [
        "--show-toplevel",
        "--absolute-git-dir",
        "cat-file",
...
fn cli_and_tracked_hook_forward_exact_stdin_arguments_and_exit_status() {
    ...
    assert!(String::from_utf8_lossy(&help.stdout).contains("--updates-stdin"));
